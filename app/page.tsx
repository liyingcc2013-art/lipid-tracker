    ...
    try {
      ...
      try {
        ...
        setReportData({
          ...
          categories: Array.isArray(extractJson.categories) ? extractJson.categories : [],
        });
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-16">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {!reportData ? (
          /* NO REPORT LOADED YET STATE */
          <section className="bg-white rounded-2xl border border-dashed border-slate-300 p-12 text-center max-w-2xl mx-auto space-y-4">
            ...
            <div className="space-y-1">
              <h3 className="text-lg font-bold text-slate-800">No Lab Report Loaded</h3>
              <p className="text-sm text-slate-500 max-w-md mx-auto">
                Please upload a lab report PDF file above to analyze and extract test results strictly from your document.
              </p>
            </div>
          </section>
        ) : (
          /* DYNAMIC REPORT DISPLAY SECTION */
          <>
            ...
            <section className="space-y-8 pt-2">
              ...
              {reportData.categories.length === 0 ? (
                <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center text-slate-500 space-y-2">
                  ...
                  <p className="text-sm font-medium">No test panels were extracted from this report text.</p>
                </div>
              ) : (
                /* RENDER CATEGORY PANELS & TEST CARDS EXCLUSIVELY FROM PAYLOAD */
                  ...
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    <div>
                      <h4 className="text-sm font-semibold text-slate-900 flex items-center gap-2">
                        ...
                        Multi-Panel Health Summary
                      </h4>
                      <p className="text-xs text-slate-500">
                        Extracted metric details across parsed panels
                      </p>
                    </div>
                  ...
                  <div className="h-56 rounded-xl bg-slate-50/70 border border-dashed border-slate-200 flex flex-col items-center justify-center relative overflow-hidden">
                    <div className="relative z-10 text-center space-y-2 p-4">
                      ...
                      <p className="text-sm font-semibold text-slate-700">
                        Patient {reportData.patient_name || 'N/A'} {reportData.patient_ic ? `(${reportData.patient_ic})` : ''}
                      </p>
                      <p className="text-xs text-slate-500 max-w-sm">
                        Loaded {reportData.categories.length} category panel(s) with total{' '}
                        {reportData.categories.reduce((acc, c) => acc + c.tests.length, 0)} test item(s) for test date{' '}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-3 text-xs text-slate-600">
                      <div className="p-3 bg-teal-50/50 rounded-xl border border-teal-100 space-y-1">
                        <p className="font-semibold text-teal-900">1:1 Accuracy Standard</p>
                        <p className="text-slate-600">
                          Extracted key metrics across {reportData.categories.length} panel(s) without synthetic data or hallucinated default values.
                        </p>
                      </div>
                    </div>
                  </div>
